...
{
    /// <summary>
    /// 서버와 클라이언트에서 공용으로 사용하는 세션 객체.
    /// 서버일 경우 :
    ///     하나의 클라이언트 객체를 나타낸다.
    ///     이 인터페이스를 구현한 객체를 CNetworkService 클래스의 session_created_callback 호출 시 생성하여 리턴시켜 준다.
    ...
    public interface IPeer
    {
        ...
        ///     
        /// 주의할 점
        ///     이 메서드에서 다른 CPeer객체를 참조하거나 공유자원에 접근할 때는 동기화 처리를 해주어야 한다.
        ///     메서드가 리턴되면 buffer는 비워지며 다음 패킷을 담을 준비를 하기 때문에 메서드를 리턴하기 전에 사용할 데이터를 모두 빼내야 한다.
        ///     다른 객체에 넘긴 buffer 참조를 메서드가 리턴된 이후에 사용하게 해서는 안된다. 이런 경우에는 참조 대신 볼사하여 처리한다.
        /// </summary>
